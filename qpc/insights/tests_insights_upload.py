...
class InsightsUploadCliTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.success_json = {
            ...
            "hosts": {"2f2cc1fd-ec66-4c67-be1b-171a595ce319": {"bios_uuid": "value"}},
        }
    ...
    def test_insights_upload_valid_report(self, subprocess):
        ...
        with requests_mock.Mocker() as mocker:
            ...
            args = Namespace(report_id="1", scan_job=None, input_file=None, no_gpg=True)
        ...
        with requests_mock.Mocker() as mocker:
            ...
            with self.assertRaises(SystemExit):
                with redirect_stdout(report_out):
                    nac.main(args)
            self.assertIn(
                (messages.BAD_INSIGHTS_UPLOAD % ("1", ""))[:10],
                report_out.getvalue().strip(),
            )
    ...
    def test_insights_upload_valid_scan_job(self, subprocess):
        ...
        with requests_mock.Mocker() as mocker:
            ...
            nac = InsightsUploadCommand(SUBPARSER)
            args = Namespace(
                scan_job_id="1", report_id=None, input_file=None, no_gpg=None
            )
            with redirect_stdout(report_out):
                nac.main(args)
                self.assertIn(
        ...
        with requests_mock.Mocker() as mocker:
            ...
            with self.assertRaises(SystemExit):
                with redirect_stdout(report_out):
                    nac.main(args)
            self.assertIn(
                messages.REPORT_SJ_DOES_NOT_EXIST % "1", report_out.getvalue().strip()
            )
    ...
    def test_insights_upload_nonexistent_report(self, subprocess):
        ...
        with requests_mock.Mocker() as mocker:
            ...
            nac = InsightsUploadCommand(SUBPARSER)
            args = Namespace(
                report_id="1", scan_job_id=None, input_file=None, no_gpg=None
            )
            with self.assertRaises(SystemExit):
                with redirect_stdout(report_out):
                    nac.main(args)
            self.assertIn(
                messages.INSIGHTS_REPORT_NOT_FOUND % ("1"),
                report_out.getvalue().strip(),
            )
        ...
        with requests_mock.Mocker() as mocker:
            ...
            with self.assertRaises(SystemExit):
                with redirect_stdout(report_out):
                    nac.main(args)
            self.assertIn(
                messages.ERROR_INSIGHTS_VERSION.replace("%s", ""), report_out.getvalue()
            )
        ...
        with requests_mock.Mocker() as mocker:
            ...
            with self.assertRaises(SystemExit):
                with redirect_stdout(report_out):
                    nac.main(args)
            self.assertIn(
                (messages.BAD_INSIGHTS_UPLOAD % ("1", ""))[:10], report_out.getvalue()
            )
    ...
    def test_outdated_version_response(self, subprocess):
        ...
        with requests_mock.Mocker() as mocker:
            ...
            cli_error_msg = messages.BAD_CLIENT_VERSION % ("3.0.1", CLIENT_VERSION)
            self.assertIn(cli_error_msg, std_out)
            cli_error_msg = messages.BAD_CORE_VERSION % ("3.0.8", CORE_VERSION)
            self.assertIn(cli_error_msg, std_out)
    ...
    def test_verify_report_hosts(self):
        ...
        hosts = {
            "5f2cc1fd-ec66-4c67-be1b-171a595ce311": {
                "bios_uuid": "value",
                "name": "value",
            },
        ...
        invalid_host = {
            "no": "canonical facts",
            "metadata": {"key": "val", "name": {"source_name": "NSource1"}},
        }
    ...
    def test_insights_upload_valid_report_from_file(self, subprocess):
        ...
        nac = InsightsUploadCommand(SUBPARSER)
        args = Namespace(
            report_id=None, scan_job=None, input_file=self.dest_tar_file, no_gpg=True
        )
        with redirect_stdout(report_out):
            nac.main(args)
            self.assertIn(
    ...
    def test_insights_upload_invalid_local_tar_path(self, subprocess):
        ...
        nac = InsightsUploadCommand(SUBPARSER)
        args = Namespace(
            report_id=None, scan_job=None, input_file="your_face_is_a", no_gpg=True
        )
        with self.assertRaises(SystemExit):
            with redirect_stdout(report_out):
                nac.main(args)
        self.assertIn(
            messages.INSIGHTS_LOCAL_REPORT_NOT % "your_face_is_a",
            report_out.getvalue().strip(),
        )
    ...
    def test_insights_upload_not_tar_extension(self, subprocess):
        ...
        nac = InsightsUploadCommand(SUBPARSER)
        args = Namespace(
            report_id=None, scan_job=None, input_file=random_file, no_gpg=True
        )
        with self.assertRaises(SystemExit):
            with redirect_stdout(report_out):
                nac.main(args)
        self.assertIn(
            messages.INSIGHTS_LOCAL_REPORT_NOT_TAR_GZ % random_file,
            report_out.getvalue().strip(),
        )
